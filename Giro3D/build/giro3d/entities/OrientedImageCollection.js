...
import { Box3, BufferGeometry, Color, DoubleSide, Euler, Float32BufferAttribute, Group, InstancedMesh, MathUtils, Matrix4, Mesh, MeshBasicMaterial, PlaneGeometry, Sphere, SphereGeometry, SRGBColorSpace, Vector3 } from 'three';
 ...
 */
export default class OrientedImageCollection extends Entity3D {
  /** Readonly flag to indicate that this object is a OrientedImageCollection instance. */
  isOrientedImageCollection = true;
  type = 'OrientedImageCollection';
  ...
  constructor(options) {
    ...
    this._images = {
      container: new Group(),
      bufferGeometry: new PlaneGeometry().applyMatrix4(new Matrix4().makeTranslation(0, 0, -1)).rotateX(MathUtils.degToRad(90)),
    ...
    frustumGeometry.setAttribute('position', new Float32BufferAttribute([0, 0, 0, -0.5, -0.5, -1, +0.5, -0.5, -1, +0.5, +0.5, -1, -0.5, +0.5, -1], 3));
  ...
  computeFrustums() {
    ...
    const instancedMesh = new InstancedMesh(this._frustums.bufferGeometry, this._frustums.material, this.source.images.length);
  ...
  computeFrustumMatrix(source) {
    ...
    const scaleMatrix = new Matrix4().makeScale(distance * frustumSizeY * source.aspectRatio, distance * frustumSizeY, distance);
