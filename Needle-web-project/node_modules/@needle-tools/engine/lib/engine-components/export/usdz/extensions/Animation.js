...
//@ts-ignore
import { buildMatrix } from "three/examples/jsm/exporters/USDZExporter";
    ...
    onSerialize(writer, _context) {
        ...
        */
        ...
        for (const transformData of arr) {
            ...
            if (!timesArray || transformData.rot && transformData.rot.times?.length > timesArray?.length)
                timesArray = transformData.rot?.times;
            if (!timesArray || transformData.scale && transformData.scale.times?.length > timesArray?.length)
                timesArray = transformData.scale?.times;
            if (!timesArray) {
