import { AnimatorConditionMode, AnimatorControllerParameterType, AnimatorStateInfo, createMotion } from "../engine/extensions/NEEDLE_animator_controller_model";
...
class RootMotionAction {
    ...
    handlePosition(_clip, track) {
        if (track) {
            ...
            if (!RootMotionAction.lastObjPosition[root.uuid])
                RootMotionAction.lastObjPosition[root.uuid] = new Vector3();
    ...
    handleRotation(clip, track) {
        if (track) {
            ...
            const root = this.root;
            if (!RootMotionAction.lastObjRotation[root.uuid])
                RootMotionAction.lastObjRotation[root.uuid] = new Quaternion();
