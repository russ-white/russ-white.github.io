...
export function getOrAddComponent(go, typeName) {
    const comp = getComponent(go, typeName);
    if (comp)
        return comp;
    const newInstance = new typeName();
    return addNewComponent(go, newInstance);
}
...
export function moveComponentInstance(obj, componentInstance) {
...
export function getComponents(obj, componentType, arr) {
    if (!arr)
        arr = [];
...
export function getComponentsInChildren(obj, componentType, arr) {
    if (!arr)
        arr = [];
...
export function getComponentsInParent(obj, componentType, arr) {
    if (!arr)
        arr = [];
    if (!obj)
        return arr;
...
export function findObjectOfType(type, contextOrScene, includeInactive) {
    if (!type)
        return null;
    if (!contextOrScene) {
...
export function findObjectsOfType(type, array, contextOrScene) {
    if (!type)
        return array;
    if (!contextOrScene) {
        contextOrScene = Context.Current;
